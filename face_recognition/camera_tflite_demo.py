...
def load_faces(faces_dir, mtcnn_detector, interpreter = None):
    face_db = []
    with tf.Graph().as_default():
        ...
        for root, dirs, files in os.walk(faces_dir):
            ...
            for file in files:
                ...
                interpreter.set_tensor(input_details[0]['index'], feed_dict[inputs_placeholder])
                interpreter.invoke()
                emb_array = interpreter.get_tensor(output_details[0]['index'])
...
def main():
    ...
    QUANTIZED_MODEL_PATH = conf.get("MOBILEFACENET", "QUANTIZED_MODEL_PATH")
    ...
    faces_db = load_faces(FACE_DB_PATH, mtcnn_detector, interpreter = interpreter)
        ...
        while True:
            ...
            if ret:
                ...
                if faces.shape[0] is not 0:
                    input_images = np.zeros((faces.shape[0], 112,112,3), dtype= np.float32)
                    for i, face in enumerate(faces):
                    ...
                    for input_image in input_images:
                        interpreter.set_tensor(input_details[0]['index'], np.expand_dims(input_image, 0))
                        interpreter.invoke()
                        emb_array = interpreter.get_tensor(output_details[0]['index'])
